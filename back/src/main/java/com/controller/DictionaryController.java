...
//            return R.error(511,"永远不会进入");
    ...
    */
    ...
    /**
     * 小程序 - 按dicCode查询字典列表（无需登录）
     */
    @IgnoreAuth
    @GetMapping("/wx/list")
    public R wxList(@RequestParam String dicCode) {
        List<DictionaryEntity> list = dictionaryService.selectList(
                new EntityWrapper<DictionaryEntity>().eq("dic_code", dicCode).orderBy("code_index", true)
        );
        List<Map<String, Object>> result = new ArrayList<>();
        for (DictionaryEntity d : list) {
            Map<String, Object> item = new HashMap<>();
            item.put("codeIndex", d.getCodeIndex());
